...
describe('getSignedCookie', () => {
  it('should read the value from the request', () => {
    ...
    expect(getSignedCookie(testSecret)(testCookieName)(req)).toEqual(testCookieValue)
  })
  it('should return undefined if the jwtToken is incorrect', () => {
    const req = mockRequest()
    expect(getSignedCookie('thisIsNotTheRightSecret')(testCookieName)(req)).toBeUndefined()
  })
  it('should return undefined if the cookie is missing', () => {
    const req = mockRequest()
    expect(getSignedCookie(testSecret)('nonExistingCookie')(req)).toBeUndefined()
  })
})